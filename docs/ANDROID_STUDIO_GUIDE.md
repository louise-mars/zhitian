...
| 服务 | 用途 | 备注 |
|------|------|------|
| 和风天气 | 天气数据（15天预报/实况/小时/预警/空气质量/生活指数/降雨/城市搜索） | 主力数据源，项目专属域名 |
| 高德定位 SDK | GPS + 基站 + WiFi 混合定位 | 国内秒定位，精确到区县·街道 |
| Open-Meteo | 天气数据备用源 | 海外服务器，仅作 fallback |

### 定位策略
- 主力：高德定位 SDK（国内秒定位，精确到区县·街道）
- 高德直接返回坐标 + 城市名 + 区县名 + 街道名，无需额外反向地理编码
- 每次定位创建独立 client，完成后销毁，避免并发竞态
...
- 启动时先显示缓存（骨架屏），后台静默刷新
- 非强制刷新时利用缓存，避免重复网络请求
- 日历月份缓存 LRU 限制（最多 6 个月）

### 通知系统
- 天气通知：WorkManager 每 6 小时检查（雨雪/温差/日程联动）
- 日程提醒：OneTimeWorkRequest 定时触发（支持提前 5/15/30/60 分钟）
- Widget 自动刷新：WorkManager 每 1 小时（需网络）
