...
- 温度阈值：≤0°C（冰冻）/ >35°C（高温）
- 预警卡片位于首页最醒目位置（诗词下方、小时预报上方）
- 气象灾害预警使用颜色边框（红/橙/黄/蓝）区分严重程度
...
- 暖金色衬线字体突出显示
- 一周内不重复
- 诗词桌面 Widget（每日自动刷新，暖金色文字 + 深色背景）

### 视觉与动画（v1.2 新增）
- 天气图标微动画（64dp Canvas：太阳脉冲/云漂移/雨滴/雪花/闪电）
- 背景渐变转场动画（700ms ease-in-out，城市切换/刷新时平滑过渡）
- 动画降级管理（低电量 <15% / 省电模式自动禁用，>20% 恢复）
- 尊重系统 Reduced Motion 无障碍设置
- 宫崎骏风格天气动画（丁达尔光束/手绘雨滴/萤火虫/季节花瓣）
- 骨架屏加载动画
- 温度趋势折线图（金黄+淡紫双线）
- 深色/浅色模式（跟随系统/手动切换，影响渐变背景色调）

### 其他
- 天气卡片图片分享（支持微信/微博等社交 App，带 fallback 到文本分享）
- 桌面天气 Widget（每小时自动刷新）
- 桌面诗词 Widget（每日自动刷新）
- 城市搜索与管理（搜索/添加/删除确认）
- 错误状态下保留导航入口
