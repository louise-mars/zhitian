...
初版 AlertEngine 对所有事件 × 所有恶劣天气都生成预警，导致大量无意义提醒：
- "Zoom 周会"也提示"建议带伞"（室内活动）
- 毛毛雨（降水 2mm）也触发预警（不值得打扰）
- 晚上暴雨但上午会议也预警（时间不匹配）
...
1. **室内事件关键词过滤**：20+ 关键词（Zoom/Teams/线上/钉钉/飞书/居家/wfh...），匹配到直接跳过
2. **降水量阈值**：`precip > 10mm` 才触发，毛毛雨不打扰。天气标签也更精确（大雨/中雨）
3. **小时级精确匹配**：今天的定时事件检查 ±1 小时天气，避免"晚上暴雨但上午会议"的误报

### 教训

预警系统的核心指标不是"覆盖率"而是"精确率"。一条误报比十条漏报更伤害用户信任。宁可漏掉一些边缘情况，也不要让用户觉得"这个提醒没用"然后关掉通知权限。
...
天气动画（粒子系统 + 图标微动画 + 背景转场）在低端设备或低电量时可能导致卡顿和耗电。

### 解决方案

`AnimationDegradationManager` 统一管理降级策略：
...
关键设计：
- **15%/20% 滞后区间**：避免电量在阈值附近时动画频繁开关
- **系统 Reduced Motion**：`ANIMATOR_DURATION_SCALE == 0` 时强制静态
- **Compose 生命周期**：`rememberInfiniteTransition` 自动在后台暂停

### 教训

工具类 App 的动画必须有"退出机制"。用户选择省电模式意味着他们明确表达了"性能优先于视觉"的偏好，App 必须尊重这个选择。
...
气象灾害预警（气象局官方）最初放在"第二屏"（需要滚动才能看到），用户容易忽略。日程天气预警也放在降雨预报下方，不够醒目。
...
```
紧急信息（预警）> 决策信息（日程冲突）> 参考信息（预报）
```

具体调整：
1. 气象灾害预警提升到诗词下方第一位（最醒目位置）
2. 日程天气预警紧跟其后
3. 气象预警使用颜色边框（红/橙/黄/蓝）+ 背景色调，视觉权重最高
4. 通知推送从 2 天扩展到 3 天（给用户更多准备时间）

### 教训

信息架构的核心原则：**紧急程度决定位置**。用户打开天气 App 的第一需求是"有没有需要我立刻注意的事"，而不是"明天几度"。预警信息必须在第一屏可见，不能让用户滚动才能发现。
